import React from 'react';
import PropTypes from 'prop-types';

import style from 'paraviewweb/style/ReactProperties/CellProperty.mcss';
import styleProp from 'paraviewweb/style/ReactProperties/MapProperty.mcss';
import ToggleIconButton from 'paraviewweb/src/React/Widgets/ToggleIconButtonWidget';

import ValueInList from './ValueInList';

/* eslint-disable react/no-danger */
export default class ListProperty extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      data: props.data,
      helpOpen: false,
    };

    // Bind callback
    this.valueChange = this.valueChange.bind(this);
    this.addEntry = this.addEntry.bind(this);
    this.helpToggled = this.helpToggled.bind(this);
  }

  componentWillMount() {
    const newState = {};
    if (this.props.ui.default && !this.props.data.value) {
      newState.data = this.state.data;
      newState.data.value = this.props.ui.default;
    }

    if (Object.keys(newState).length > 0) {
      this.setState(newState);
    }
  }

  componentWillReceiveProps(nextProps) {
    const data = nextProps.data;

    if (this.state.data !== data) {
      this.setState({
        data,
      });
    }
  }

  helpToggled(open) {
    this.setState({
      helpOpen: open,
    });
  }

  valueChange(idx, value) {
    const { data } = this.state;
    if (value === undefined) {
      data.value.splice(idx, 1);
    } else {
      data.value[idx].value = value;
    }
    this.setState({ data });

    if (this.props.onChange) {
      this.props.onChange(data);
    }
  }

  addEntry() {
    const { data } = this.state;
    data.value.push({ value: '' });
    this.setState({ data });
  }

  render() {
    const mapper = () => {
      if (Array.isArray(this.state.data.value)) {
        const ret = [];
        for (let i = 0; i < this.state.data.value.length; i++) {
          ret.push(
            <ValueInList
              idx={i}
              value={this.state.data.value[i]}
              onChange={this.valueChange}
            />
          );
        }
        return ret;
      }

      return null;
    };

    return (
      <div
        className={
          this.props.show(this.props.viewData) ? style.container : style.hidden
        }
      >
        <div className={style.header}>
          <strong>{this.props.ui.label}</strong>
          <span>
            <span className={styleProp.addButton} onClick={this.addEntry} />
            <ToggleIconButton
              icon={style.helpIcon}
              value={this.state.helpOpen}
              toggle={!!this.props.ui.help}
              onChange={this.helpToggled}
            />
          </span>
        </div>
        <div className={style.inputBlock}>
          <table className={styleProp.table}>
            <tbody> {mapper()} </tbody>
          </table>
        </div>
        <div
          className={this.state.helpOpen ? style.helpBox : style.hidden}
          dangerouslySetInnerHTML={{ __html: this.props.ui.help }}
        />
      </div>
    );
  }
}
/* eslint-enable react/no-danger */

ListProperty.propTypes = {
  data: PropTypes.object.isRequired,
  help: PropTypes.string,
  onChange: PropTypes.func.isRequired,
  show: PropTypes.func.isRequired,
  ui: PropTypes.object.isRequired,
  viewData: PropTypes.object.isRequired,
};

ListProperty.defaultProps = {
  help: '',
};
